...
KEYWORDS = [
    ...
    # Entry-level signals (optional but useful for students)
    "fresh graduate it", "entry level it", "junior developer", "magang it",
]

# Soft preferences (set to [] to disable)
PREFERRED_LOCATIONS = [
...
# GENERIC location from meta (tanpa daftar kota statis)
...
def enrich_from_detail(session: requests.Session, headers: dict, link: str):
    ...
    # LOCATION (prioritas lokasi detail)
    ...
    if not out["location"] or is_noise(out["location"], 120):
        ...
        if loc_sel and not is_noise(loc_sel, 120):
            ...
            # GENERIC fallback: cari frasa "A, B" atau "A, B, C"
            best = ""
...
def keep_row(rec: dict) -> bool:
    ...
    if PREFERRED_WORKTYPE_RX and work and not PREFERRED_WORKTYPE_RX.search(work):
        # comment the next line if you want to keep all work types
        # return False
        pass

    if PREFERRED_LOCATION_RX and loc and not PREFERRED_LOCATION_RX.search(loc):
        # comment the next line if you want to keep all locations
        # return False
        pass

    return True

# --------- Level inference (optional) ----------
...
@profile
def scrapeData(tableName: str = "public.scrape_items"):
    start = time.time()
    ...
    for kw in KEYWORDS:
        ...
        while True:
            ...
            if not cards:
                print("[INFO] Tidak menemukan job cards di halaman ini. Stop di page", page)
                break
            ...
            for card in cards:
                ...
                if not link or link in seen_links:
                    continue
                seen_links.add(link)
                # Save kategori = originating keyword
                rows.append({"title": title, "link": link, "kategori": kw})
                added_this_page += 1

            if added_this_page == 0:
                print("[INFO] Tidak ada link baru di page", page, "— selesai untuk keyword ini.")
                break
    ...
    enriched = []
    for r in tqdm(rows, desc="Enrich detail"):
    ...
    # ---- save to DB (optional) ----
    try:
        import db_mysql  # local import so this file runs even if DB module absent
